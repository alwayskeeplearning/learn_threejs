...
rgbeLoader.load('/static/textures/Video_Copilot-Back Light_0007_4k.hdr', texture => {
  ...
  dirLightFolder.add(directionalLight, 'visible').name('开启/关闭');
  dirLightFolder.add(directionalLight, 'intensity').name('强度').min(0).max(10).step(0.1);
  dirLightFolder.addColor(directionalLight, 'color').name('颜色');
  dirLightFolder.add(directionalLight.position, 'x').name('X').min(-10).max(10).step(0.1);
  dirLightFolder.add(directionalLight.position, 'y').name('Y').min(-10).max(10).step(0.1);
  dirLightFolder.add(directionalLight.position, 'z').name('Z').min(-10).max(10).step(0.1);
  const dirLightTargetFolder = dirLightFolder.addFolder('目标');
  dirLightTargetFolder
